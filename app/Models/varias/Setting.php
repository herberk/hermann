...
class Setting extends Model
{
  ...
  //  protected $guarded = [];
    ...
    public static function add($key, $val, $type = 'string', $empresas_id)
    {
        if ( self::has($key) ) {
            return self::set($key, $val, $type, $empresas_id);
        }

        return self::create(['name' => $key, 'val' => $val, 'type' => $type, 'empresas_id'=> $empresas_id  ]) ? $val : false;
    }
     ...
     * @return bool
     */
    public static function set($key, $val, $type = 'string', $empresas_id)
    {
        ...
        if ( $setting = self::getAllSettings()->where('name', $key)->first() ) {
            return $setting->update([
                'name' => $key,
                'val' => $val,
                'empresas_id' => $empresas_id, //     $empresa_id = $val['empresa_id'];
                'type' => $type]) ? $val : false;
        }

       return self::add($key, $val, $type, $empresas_id);
    }
